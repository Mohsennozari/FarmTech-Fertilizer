  ...
  <div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-5 border border-gray-200">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-lg p-3 text-center shadow-sm">
          <svg class="w-6 h-6 mx-auto text-green-600 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="bg-white rounded-lg p-3 text-center shadow-sm">
          <svg class="w-6 h-6 mx-auto text-blue-600 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="font-bold text-gray-800">{{ formatNumber(result.tank_main_result?.tank_volume_liters || 0) }} <span class="text-sm font-normal">لیتر</span></div>
        </div>
        <div class="bg-white rounded-lg p-3 text-center shadow-sm">
          <svg class="w-6 h-6 mx-auto text-amber-600 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="font-bold text-gray-800">{{ formatNumber(result.tank_calcium_result?.tank_volume_liters || 0) }} <span class="text-sm font-normal">لیتر</span></div>
        </div>
        <div class="bg-white rounded-lg p-3 text-center shadow-sm">
          <svg class="w-6 h-6 mx-auto text-purple-600 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.414 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />
          </svg>
          <div class="text-xs text-gray-500">حجم مخزن استوک</div>
          <div class="font-bold text-gray-800">{{ props.stockTankVolume || 20 }} <span class="text-sm font-normal">لیتر</span></div>
        </div>
        <div class="bg-white rounded-lg p-3 text-center shadow-sm">
          <svg class="w-6 h-6 mx-auto text-indigo-600 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
          <div class="text-xs text-gray-500">نسبت تزریق</div>
          <div class="font-bold text-gray-800">1:{{ props.injectorRatio || 200 }}</div>
        </div>
        <div class="bg-white rounded-lg p-3 text-center shadow-sm">
          <svg class="w-6 h-6 mx-auto text-gray-600 mb-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
          </svg>
          <div class="text-xs text-gray-500">حجم کل محلول نهایی</div>
          <div class="font-bold text-gray-800">{{ ((result.tank_main_result?.tank_volume_liters || 0) + (result.tank_calcium_result?.tank_volume_liters || 0)).toLocaleString() }} <span class="text-sm font-normal">لیتر</span></div>
        </div>
      </div>
      
      <div class="mt-4 bg-blue-50 rounded-lg p-3 text-sm border border-blue-200">
        ...
        <p class="text-blue-700">به ازای هر 1 لیتر استوک، {{ (props.injectorRatio || 200) - 1 }} لیتر آب اضافه می‌شود تا {{ props.injectorRatio || 200 }} لیتر محلول نهایی بدست آید.</p>
      </div>
    </div>
            ...
            <tbody>
              <tr v-for="item in nutrientComparison" :key="item.element" class="hover:bg-gray-50">
                ...
                <td class="border border-gray-300 px-3 py-2 text-center">
                  <span v-if="item.status === 'ok'" class="text-green-600 font-medium">✓ کافی</span>
                  <span v-else-if="item.status === 'low'" class="text-amber-600 font-medium">⚠️ کم</span>
                  <span v-else class="text-red-600 font-medium">✗ بسیار کم</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div class="bg-white rounded-xl shadow-card border border-gray-100 overflow-hidden">
      ...
      <div class="p-5">
          ...
          <table class="w-full text-sm border-collapse">
            <thead>
              <tr class="bg-gray-100">
                ...
                <th class="border border-gray-300 px-3 py-2 text-center">مقدار برای استوک</th>
            ...
            <tbody>
              <tr v-for="dose in result.tank_main_result.doses" :key="dose.name" class="hover:bg-gray-50">
                ...
                <td class="border border-gray-300 px-3 py-2 text-center">
                  <span v-if="getStockAmount(dose) >= 1000">{{ (getStockAmount(dose) / 1000).toFixed(2) }} کیلوگرم</span>
                  <span v-else-if="getStockAmount(dose) >= 1">{{ getStockAmount(dose).toFixed(0) }} گرم</span>
                  <span v-else>{{ (getStockAmount(dose) * 1000).toFixed(0) }} میلی‌گرم</span>
                </td>
                <td class="border border-gray-300 px-3 py-2 text-center">
                  <button @click="showFertilizerDetail(dose.name)" class="text-blue-600 hover:text-blue-800 transition" title="مشاهده اطلاعات کامل کود">
        ...
        <div class="bg-purple-50 rounded-lg p-3 mb-3">
          ...
          <p class="text-sm text-gray-700 mb-2">برای ساخت {{ props.stockTankVolume || 20 }} لیتر استوک با نسبت 1:{{ props.injectorRatio || 200 }}، مقادیر زیر را با دقت اندازه گیری کنید:</p>
          <div class="bg-white rounded-lg p-3">
            <div v-for="dose in result.tank_main_result.doses" :key="dose.name" class="flex justify-between items-center py-1 border-b border-gray-100 last:border-0">
              <span class="text-sm font-medium">{{ dose.persian_name || dose.name }}</span>
        ...
        <div class="bg-green-50 rounded-lg p-3 mb-3">
          ...
          <div class="grid grid-cols-2 gap-3 mb-2">
            <div class="bg-white rounded-lg p-2 text-center">
              <p class="text-lg font-bold text-green-700">{{ ((result.tank_main_result.tank_volume_liters || 0) / (props.injectorRatio || 200)).toFixed(1) }} لیتر</p>
              <p class="text-xs text-gray-600">استوک برای مخزن {{ formatNumber(result.tank_main_result.tank_volume_liters || 0) }} لیتری</p>
            </div>
            <div class="bg-white rounded-lg p-2 text-center">
              <p class="text-lg font-bold text-green-700">{{ (1000 / (props.injectorRatio || 200)).toFixed(1) }} میلی‌لیتر</p>
    ...
    <div class="bg-white rounded-xl shadow-card border border-gray-100 overflow-hidden">
      ...
      <div class="p-5">
          ...
          <table class="w-full text-sm border-collapse">
            ...
            <tbody>
              <tr v-for="dose in result.tank_calcium_result.doses" :key="dose.name" class="hover:bg-gray-50">
                ...
                <td class="border border-gray-300 px-3 py-2 text-center">
                  <button @click="showFertilizerDetail(dose.name)" class="text-amber-600 hover:text-amber-800 transition" title="مشاهده اطلاعات کامل کود">
                    <svg class="w-5 h-5 inline" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                      <circle cx="12" cy="12" r="10" />
                      <path d="M12 16v-4M12 8h.01" />
                    </svg>
        ...
        <div class="bg-purple-50 rounded-lg p-3 mb-3">
          ...
          <p class="text-sm text-gray-700 mb-2">برای ساخت {{ props.stockTankVolume || 20 }} لیتر استوک کلسیم با نسبت 1:{{ props.injectorRatio || 200 }}، مقادیر زیر را با دقت اندازه گیری کنید:</p>
          <div class="bg-white rounded-lg p-3">
            <div v-for="dose in result.tank_calcium_result.doses" :key="dose.name" class="flex justify-between items-center py-1 border-b border-gray-100 last:border-0">
              <span class="text-sm font-medium">{{ dose.persian_name || dose.name }}</span>
              <span class="text-sm text-gray-600">
                <span v-if="getStockAmount(dose) >= 1000">{{ (getStockAmount(dose) / 1000).toFixed(2) }} کیلوگرم</span>
                <span v-else-if="getStockAmount(dose) >= 1">{{ getStockAmount(dose).toFixed(0) }} گرم</span>
        ...
        <div class="bg-green-50 rounded-lg p-3 mb-3">
          ...
          <div class="grid grid-cols-2 gap-3 mb-2">
            <div class="bg-white rounded-lg p-2 text-center">
              <p class="text-lg font-bold text-green-700">{{ ((result.tank_calcium_result.tank_volume_liters || 0) / (props.injectorRatio || 200)).toFixed(1) }} لیتر</p>
              <p class="text-xs text-gray-600">استوک برای مخزن {{ formatNumber(result.tank_calcium_result.tank_volume_liters || 0) }} لیتری</p>
            </div>
            <div class="bg-white rounded-lg p-2 text-center">
              <p class="text-lg font-bold text-green-700">{{ (1000 / (props.injectorRatio || 200)).toFixed(1) }} میلی‌لیتر</p>
              <p class="text-xs text-gray-600">استوک برای هر 1 لیتر آب</p>
        ...
        <div class="bg-red-50 rounded-lg p-3 mb-3">
          <p class="text-sm text-red-700 flex items-center gap-2">
            ...
            هیچگاه استوک مخزن کلسیم را با استوک مخزن اصلی قبل از ورود به مخزن اصلی مخلوط نکنید!
          </p>
        </div>
    ...
    <div class="bg-yellow-50 rounded-xl p-4 border border-yellow-200">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
        ...
        <div class="bg-white rounded-lg p-2">
          <span class="font-bold">ماندگاری در دمای محیط:</span> 3 روز (دمای زیر 25 درجه)
        </div>
        <div class="bg-white rounded-lg p-2 col-span-2">
          <span class="font-bold">نشانه‌های خرابی:</span> رسوب سفید رنگ، تغییر رنگ، بوی نامطبوع، باد کردگی ظرف
        </div>
      </div>
    </div>
      ...
      <div class="bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-hidden shadow-xl">
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(85vh-70px)]">
          ...
          <div v-else-if="fertilizerDetail" class="space-y-4">
            <div>
              <h4 class="font-bold text-gray-800 text-lg">{{ fertilizerDetail.persian_name || fertilizerDetail.name }}</h4>
              <div class="flex gap-2 mt-1">
                <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{{ fertilizerDetail.brand_name }}</span>
                <span class="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">{{ fertilizerDetail.fertilizer_type }}</span>
              </div>
            </div>
            
            <div class="border-t pt-3">
              <h5 class="font-semibold text-gray-700 mb-2">توضیحات</h5>
              <p class="text-sm text-gray-600 whitespace-pre-line">{{ fertilizerDetail.description || 'توضیحاتی برای این کود ثبت نشده است' }}</p>
            </div>
            
            <div class="border-t pt-3">
              <h5 class="font-semibold text-gray-700 mb-2">ترکیبات شیمیایی</h5>
              <div class="grid grid-cols-2 md:grid-cols-3 gap-2 text-sm">
                <div v-if="fertilizerDetail.n_percent">N: <span class="font-medium">{{ fertilizerDetail.n_percent }}%</span></div>
                <div v-if="fertilizerDetail.p_percent">P: <span class="font-medium">{{ fertilizerDetail.p_percent }}%</span></div>
                <div v-if="fertilizerDetail.k_percent">K: <span class="font-medium">{{ fertilizerDetail.k_percent }}%</span></div>
                <div v-if="fertilizerDetail.ca_percent">Ca: <span class="font-medium">{{ fertilizerDetail.ca_percent }}%</span></div>
                <div v-if="fertilizerDetail.mg_percent">Mg: <span class="font-medium">{{ fertilizerDetail.mg_percent }}%</span></div>
                <div v-if="fertilizerDetail.s_percent">S: <span class="font-medium">{{ fertilizerDetail.s_percent }}%</span></div>
                <div v-if="fertilizerDetail.fe_percent">Fe: <span class="font-medium">{{ fertilizerDetail.fe_percent }}%</span></div>
                <div v-if="fertilizerDetail.zn_percent">Zn: <span class="font-medium">{{ fertilizerDetail.zn_percent }}%</span></div>
                <div v-if="fertilizerDetail.mn_percent">Mn: <span class="font-medium">{{ fertilizerDetail.mn_percent }}%</span></div>
                <div v-if="fertilizerDetail.cu_percent">Cu: <span class="font-medium">{{ fertilizerDetail.cu_percent }}%</span></div>
                <div v-if="fertilizerDetail.b_percent">B: <span class="font-medium">{{ fertilizerDetail.b_percent }}%</span></div>
                <div v-if="fertilizerDetail.mo_percent">Mo: <span class="font-medium">{{ fertilizerDetail.mo_percent }}%</span></div>
              </div>
            </div>
            ...
            <div class="border-t pt-3">
              ...
              <div class="grid grid-cols-2 gap-2 text-sm">
                <div>حداقل دوز: <span class="font-medium">{{ fertilizerDetail.min_dose_g_per_liter || 'نامشخص' }} g/L</span></div>
                <div>حداکثر دوز: <span class="font-medium">{{ fertilizerDetail.max_dose_g_per_liter || 'نامشخص' }} g/L</span></div>
                <div v-if="fertilizerDetail.solubility_g_per_l">حلالیت: <span class="font-medium">{{ fertilizerDetail.solubility_g_per_l }} g/L</span></div>
                <div v-if="fertilizerDetail.ph_effect">اثر بر pH: <span class="font-medium">{{ fertilizerDetail.ph_effect }}</span></div>
              </div>
            </div>
            
            <div v-if="fertilizerDetail.registration_code" class="border-t pt-3">
              <h5 class="font-semibold text-gray-700 mb-2">اطلاعات ثبتی</h5>
              <div class="text-sm">کد ثبت: <span class="font-mono">{{ fertilizerDetail.registration_code }}</span></div>
            </div>
          </div>
...
const showFertilizerDetail = async (fertilizerName: string) => {
  ...
  try {
    const found = allFertilizers.value.find((f: any) => f.name === fertilizerName)
    if (found) {
